...
object ListExercises {
  ...
  def youngestPerson(persons: List[Person]): Person = persons match {
    ...
    case head :: tail => tail.foldLeft(head)((p1, p2) => if (p1.age <= p2.age) p1 else p2)
  }
  ...
  def showEveryNthPerson(n: Int, persons: List[Person]): List[String] = {
    persons.zipWithIndex.filter(z => (z._2 + 1) % n == 0).map(z => s"${z._1.name} is ${z._1.age} years old")

//    persons.zipWithIndex.filter(
//      case (p, i) => (index + 1) % n == 0
//    ).map(p => p match {
