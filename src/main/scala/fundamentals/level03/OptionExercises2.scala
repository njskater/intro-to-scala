...
object OptionExercises2 {
  ...
  /**
    ...
    * ```
    */
  def findJobDescriptionGivenJobId2(jobId: JobId): Option[String] = findJobById(jobId).map(job => job.description)
  ...
  /**
    ...
    */
  def findJobIdByHumanId(humanId: HumanId): Option[JobId] = findHumanById(humanId).map(h => h.optJobId).flatten
